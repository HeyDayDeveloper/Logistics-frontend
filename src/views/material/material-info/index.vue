  ...
  formRef.value?.validate(async (valid: boolean) => {
    if (valid) {
      if (currentUpdateId.value === undefined) {
        const res = await useProductStore().addData(formData)
        console.log(res)
        ElMessage.success("新增成功")
        dialogVisible.value = false
        getTableData()
      } else {
        const res = await useProductStore().updateData({ id: currentUpdateId.value, ...formData })
        console.log(res)
...
const getTableData = async () => {
  loading.value = true
  try {
    const res: [] = (await useProductStore().getData()) as []
...
const handleSearch = async () => {
  if (paginationData.currentPage === 1) {
    const res: any = await useProductStore().getSearchData(searchData.name)
...
const handleDelete = (row: any) => {
  ElMessageBox.confirm(`正在删除用户：${row.name}，确认删除？`, "提示", {
    ...
    const res = await useProductStore().deleteData(row.id)
    console.log(res)
...
const batchDelete = () => {
  deleteList.forEach(async (item) => {
    const res = await useProductStore().deleteData(item)
    console.log(res)
  })
  deleteList.clear()
  ElMessage.success("删除成功")
  ...
  <div class="app-container">
    ...
    <el-card v-loading="loading" shadow="never">
      <div class="toolbar-wrapper">
        <div>
          <el-button type="primary" :icon="CirclePlus" @click="dialogVisible = true">新增物资信息</el-button>
      ...
      <div class="table-wrapper">
        <el-table :data="tableData" @selection-change="handleSelectionChange($event)" @select-all="handleChangeAll()">
          ...
          <el-table-column prop="id" label="物品id" align="center" />
          <el-table-column prop="name" label="物品名" align="center" />
          <el-table-column prop="category.name" label="物品类别" align="center" />
    ...
    <el-dialog
      ...
      :title="currentUpdateId === undefined ? '新增物资信息' : '修改物资信息'"
      @close="resetForm"
      width="30%"
    >
      <el-form ref="formRef" :model="formData" label-width="100px" label-position="left">
        ...
        <el-form-item prop="category" label="物品类别">
        ...
        <el-form-item prop="unit" label="单位" v-if="currentUpdateId === undefined">
